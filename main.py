...
import sqlalchemy
import psycopg2
from DataBase import insert_client, insert_partner, insert_photo, insert_client_partner
...
class VkUser:
    ...
    def UsersInfo(self, id_client):
        ...
        # pprint(req)
        client_info = {}
        client_info['id_client'] = id_client
    ...
    def search_pare(self, client_info):
        ...
        if client_info['sex'] == 1:
            ...
            pare_info['sex'] = '1'
        if client_info['sex'] == 1:
            pare_info['age_from'] = 2021 - int(client_info['bdate']) + 2
            pare_info['age_to'] = 2021 - int(client_info['bdate']) + 2
        elif client_info['sex'] == 2:
            pare_info['age_from'] = 2021 - int(client_info['bdate']) - 2
            pare_info['age_to'] = 2021 - int(client_info['bdate']) - 2
        # print(pare_info)
...
def chat_bot():
    ...
    def search_pare_photos(client_info_all):
        partner_info = vk_client.search_pare(client_info_all)
        dic['partner_info'] = partner_info
        pare_id = partner_info['pare_id']
        if pare_id == 1:
    ...
    for event in longpoll.listen():
        if event.type == VkEventType.MESSAGE_NEW:
            ...
            if event.to_me:
                ...
                dic = {'client_info': client_info}
                city_list = vk_client.search_city()
                # Структура переписки
                if message == 'привет':
                    client_info_all = client_info
                    if len(client_info) == 7 and client_info_all['status'] != 0:
                        blasthack(user_id, 'Ваши данные полные, введите "найди пару"')
                    elif len(client_info) == 7 and client_info_all['status'] == 0:
                        client_info_all['status'] = 6
                        blasthack(user_id,
                                  'Ваш статус неопределен, ищем для вас партнера в активном поиске, введите "найди пару"')
                    ...
                    dic['Photo'] = {'pare_id': dic['partner_info']['pare_id'], 'url_photo': sms[0]}
                    pprint(dic)
                    with open('reserv.json', 'w', encoding='utf-8') as f:
...
def data_base_work():
    with open('reserv.json', 'r', encoding='utf-8') as f:  # открыли файл с данными
        text = json.load(f)  # загнали все, что получилось в переменную
    insert_client(text['client_info'])
    insert_partner(text['partner_info'])
    insert_photo(text['Photo'])
    insert_client_partner(text)
